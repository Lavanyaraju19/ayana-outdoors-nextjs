    ...
    <div className="container mx-auto px-4 relative z-10">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
        <h1 className="font-display text-4xl md:text-6xl text-white mb-4">
          Your Child's <span className="text-primary">Safety & Care</span>
        </h1>
        <p className="font-body text-lg text-white/80 max-w-2xl mx-auto shadow-sm">
          Safety is Our Priority. Every outdoor learning experience is planned with care, preparation, trained supervision, parent communication and age-appropriate guidance.
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {safetyItems.map((item, i) => (
          ...
          >
            ...
            {/* h2 (not h3) — keeps heading levels sequential under the page's single h1; visual size is set by the classes below, not the tag */}
      ...
      >
        <h2 className="font-display text-2xl md:text-3xl text-primary mb-4">Safety-Led Preparation</h2>
        <p className="font-body text-muted-foreground max-w-xl mx-auto">
          Every program is planned around route readiness, trained supervision, emergency response thinking, and parent communication. Specific certification and incident-record claims require client confirmation before publication.
        </p>
      </motion.div>
    </div>
  );
